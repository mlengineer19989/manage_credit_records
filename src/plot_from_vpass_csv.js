...
const CATEGORY_ORDER = ['スーパー', 'コンビニ', '自販機', '外食', '交通費', '光熱費', '娯楽・嗜好品', 'サブスク', '投資', UNCLASSIFIED_LABEL];
...
function classifyExpenses() {
  ...
  files.forEach(file => {
    ...
    rows.forEach(row => {
      ...
      const store = toFullWidthKatakana((row[1] || '').toString().trim());
...
// 半角カナで記録されているケースがあり、そのままだとCATEGORY_DICT(全角カナ)に
...
const HALFWIDTH_HANDAKUTEN_MAP = {
  'ﾊ': 'パ', 'ﾋ': 'ピ', 'ﾌ': 'プ', 'ﾍ': 'ペ', 'ﾎ': 'ポ',
};
...
function toFullWidthKatakana(str) {
  let result = str.replace(/[\uFF61-\uFF9F][\uFF9E\uFF9F]?/g, match => {
    const base = match[0];
    const mark = match[1];
    if (mark === 'ﾞ' && HALFWIDTH_DAKUTEN_MAP[base]) return HALFWIDTH_DAKUTEN_MAP[base];
    if (mark === 'ﾟ' && HALFWIDTH_HANDAKUTEN_MAP[base]) return HALFWIDTH_HANDAKUTEN_MAP[base];
    return (HALFWIDTH_KATAKANA_MAP[base] || base) + (mark || '');
  });
...
function writeTransitionSection(sheet, startRow, files, transactions) {
  ...
  const chartColumnCount = CATEGORY_ORDER.length + 1; // ファイル名 + 各分類(グラフの積み上げ対象)
  const columnCount = chartColumnCount + 1; // 上記 + 合計列
  sheet.getRange(headerRow, 1, 1, columnCount).setValues([['ファイル名'].concat(CATEGORY_ORDER).concat(['合計'])]);

  // ファイルごとに、分類の並び順(CATEGORY_ORDER)に沿って合計金額を並べ(0円の分類も含める)、
  ...
  const dataRows = files.map(file => {
    ...
    const categoryAmounts = CATEGORY_ORDER.map(category => totals[category] || 0);
